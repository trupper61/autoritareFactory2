...
namespace autoritaereFactory.world
{
    public class Chunk
    {
        ...
        public Chunk(int posX, int posY,int seed)
        {
            ...
            GenerateCircleRessource(seed + 6, GroundResource.LimeStone, 10f, 30, 1f,0.75f,0f,0.5f); // evl Kohle
            ...
            for (int ptX = 0; ptX < chunkSize; ptX++)
            {
                for (int ptY = 0; ptY < chunkSize; ptY++)
                {
                    ...
                    if (blockState[ptX, ptY] == GroundResource.LimeStone)//LimeStone
                        blockState[ptX, ptY] = (GroundResource)rng.Next(
                            (int)GroundResource.LimeStone, (int)GroundResource.LimeStoneUpperBound); // Desert 1 && Desert 2
                }
            }
        }
        private Chunk()
