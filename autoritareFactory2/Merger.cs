...
namespace factordictatorship
{
    internal class Merger : Fabrikgebeude
    {
        ...
        public override void Iteration()
        {
            for (int i = 0; i < 4; i++)
            {
                nimmVomBand(listResource, 0, 1, maxAnzalRecurse1, 3);//Eingang: unten, links, und oben. Ausgang: Rechts
                nimmVomBand(listResource, -1, 0, maxAnzalRecurse1, 0);
                nimmVomBand(listResource, 0, -1, maxAnzalRecurse1, 1);
        ...
        public static new Merger FromByteArray(byte[] bytes, ref int offset)
        {
            ...
            {
                newMerger.listResource.Add(new Resource((ResourceType)BitConverter.ToInt32(bytes, offset)));
                offset += 4;
            }
            return newMerger;
        }
